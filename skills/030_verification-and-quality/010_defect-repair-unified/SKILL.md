...
- 「なんとなく直した」を排除: 原因仮説・影響範囲・複数対応案を体系化し、同じ問題の再発を防ぐ
- 調査 → 決定 → 実装 → テストの順序を守る: 実装から始めると誤認した原因のまま直すリスクがある
- 開発者が毎回「対応案を選ぶ」承認を行う: AI の修正に意図しない変更が混入しないための安全弁
...
- 前提: なし（このスキルから始められる）
- 次: [030_code-review-assistant](../../020_design-and-implementation/030_code-review-assistant/SKILL.md)（改修後のコードレビュー標準化）
- 関連: [010_requirements-refinement](../../010_requirements-and-planning/010_requirements-refinement/SKILL.md)（不具合の根本が仕様の曖昧さに起因する場合）

## 利用する場面
- 機能不具合が発生し、原因から対応、テストまでを体系的に進めたい
...
- [ ] 「症状」（何が起きているか）と「影響範囲」（どこで起きているか）を区別して説明できる
- [ ] ログ・スタックトレース・再現手順が手元にある、または入手方法がわかる
- [ ] 本番緊急対処（ロールバック等）はこのスキルのスコープ外と確認している

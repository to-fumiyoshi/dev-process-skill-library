...
- セキュリティは「後から追加する」のが最も難しい。設計段階に組み込まないと構造的な穴が残る
- 脅威を先に列挙する。実装後の対策は対症療法になり、設計上の問題は残り続ける
- 攻撃者視点（何を狙うか）と防御視点（何を守るか）の両方を持つことが本質
...
- セットで使う: [010_api-contract-design](../../020_design-and-implementation/010_api-contract-design/SKILL.md)（API 設計時のセキュリティ確認）
- 次: [030_release-readiness](../../040_operations-and-release/030_release-readiness/SKILL.md)（リリース前の最終確認として）
...
- [ ] 保護対象データ（個人情報・認証情報等）を列挙できる
- [ ] 認証（本人確認）と認可（権限確認）の違いを説明できる
- [ ] このスキルの対象は「実装そのもの」でなく「セキュリティ要件の整理と確認」と理解している

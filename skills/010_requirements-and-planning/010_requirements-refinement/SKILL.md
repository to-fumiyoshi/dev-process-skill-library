...
- 要求が曖昧なまま実装を始めると「作り直し」が発生する。後から修正するコストは早期の10倍になる
- 受入条件を先に確定する。実装中の変更はスコープ膨張と連鎖コストを生む
- 「判定可能な受入条件」を書けなければ、何を作れば完成か誰にもわからない
...
- 前提: なし（開発の出発点となるスキル）
- 次: [040_data-model-design-unified](../../020_design-and-implementation/040_data-model-design-unified/SKILL.md)（データ設計が必要なとき）
- 次: [010_api-contract-design](../../020_design-and-implementation/010_api-contract-design/SKILL.md)（API 先行設計が必要なとき）
...
- [ ] 達成したい成果を一言で説明できる
- [ ] 受入条件を「テストで合否を判定できる文」として書ける
- [ ] スコープ外（作らないもの）を少なくとも1つ挙げられる
